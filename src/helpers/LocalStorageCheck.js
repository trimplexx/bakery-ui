export const LocalStorageCheck = () => {
    ...
    // Sprawdzenie, czy wybrana data jest starsza niż dzisiejszy dzień lub czy obecny czas jest po godzinie 16:45
    const now = new Date();
    now.setMinutes(now.getMinutes() - now.getTimezoneOffset());
    if (selectedDate < today || (now.getHours() >= 17 || (now.getHours() === 16 && now.getMinutes() >= 45))) {

        // Jeśli tak, ustaw datę na obecny lub następny dzień
        const newDate = (now.getHours() >= 17 || (now.getHours() === 16 && now.getMinutes() >= 45)) ? new Date(now.setDate(now.getDate() + 1)) : now;
    ...
    if(now.getDate() === 6)
    {
        if (selectedDate < today || (now.getHours() >= 14 || (now.getHours() === 13 && now.getMinutes() >= 45)) ) {
