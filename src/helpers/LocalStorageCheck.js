export const LocalStorageCheck = () => {
    ...
    if (selectedDate < today || (now.getHours() > 15 || (now.getHours() === 15 && now.getMinutes() >= 45))) {

        // Jeśli tak, ustaw datę na obecny lub następny dzień
        const newDate = (now.getHours() > 15 || (now.getHours() === 15 && now.getMinutes() >= 45)) ? new Date(now.setDate(now.getDate() + 1)) : now;
