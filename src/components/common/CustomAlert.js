...
export const CustomAlert = ({isVisible, type, info, handleClose}) => {
    ...
    return (
        <div className={`flex items-center p-4 mb-4 ${colors.text} rounded-lg ${colors.bg}`} role="alert">
            ...
            <button type="button" onClick={handleClose} className={`ms-auto -mx-1.5 -my-1.5 ${colors.buttonBg} ${colors.text} rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 ${colors.buttonHover} inline-flex items-center justify-center h-8 w-8`} aria-label="Close">
